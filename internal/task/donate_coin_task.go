...
func doDonateCoin(ctx context.Context) {
	...
	if coins <= float64(config.ReserveCoins) {
		log.Errorf("账户余额不足%d，无法进行投币任务\n", config.ReserveCoins)
		return
	}

	if details.Level >= 6 {
		log.Infoln("账号已到达6级，取消执行投币任务")
		return
	}

	expToday, err := d.GetCoinExpToday()
	if err != nil {
		log.WithError(err).Errorln("获取今日投币经验失败")
		return
	}

	needCoins := config.DonateCoins - expToday/10
	if needCoins <= 0 {
		log.Infoln("今日投币任务已完成")
		return
	}

	trendVideo, err := d.GetTrendVideo(regionIds[rand.Intn(len(regionIds))])
	if err != nil {
		log.WithError(err).Errorln("获取分区视频排行失败")
		return
	}
	...
	for _, v := range trendVideo {
		...
		if err != nil {
			log.WithError(err).Errorf("为视频[%s]投币失败\n", v.Title)
			return
		}
		log.Infof("为视频[%s]投币成功\n", v.Title)
		needCoins--
	}
}
